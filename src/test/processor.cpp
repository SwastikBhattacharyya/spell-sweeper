...
TEST(processor_test, split_input) {
  ...
  EXPECT_EQ(split_test[1][0], "");
  EXPECT_EQ(split_test[1][1], " ");
  EXPECT_EQ(split_test[1][2], "");
  ...
  EXPECT_EQ(split_test[7][0], "");
  EXPECT_EQ(split_test[7][1], "\n");
  EXPECT_EQ(split_test[7][2], "");
  EXPECT_EQ(split_test[8][0], "$%^");
  EXPECT_EQ(split_test[8][1], "asd");
  EXPECT_EQ(split_test[8][2], "$$");
}

TEST(processor_test, join_split_words) {
  std::string test = "'This is\"\" a '\n$%^asd$$";
  std::vector<std::array<std::string, 3>> split_test =
      spell_sweeper::processor::split_input(test);

  std::string joined = spell_sweeper::processor::join_split_words(split_test);

  EXPECT_EQ(test, joined);
}
...
TEST(processor_test, join_word) {
  std::string word1 = ".$%word.$'";
  std::array<std::string, 3> word1_split =
      spell_sweeper::processor::split_word(word1);
  EXPECT_EQ(spell_sweeper::processor::join_word(word1_split), word1);

  std::string word2 = "\"'word'\"";
  std::array<std::string, 3> word2_split =
      spell_sweeper::processor::split_word(word2);
  EXPECT_EQ(spell_sweeper::processor::join_word(word2_split), word2);

  std::string word3 = "word";
  std::array<std::string, 3> word3_split =
      spell_sweeper::processor::split_word(word3);
  EXPECT_EQ(spell_sweeper::processor::join_word(word3_split), word3);
}
