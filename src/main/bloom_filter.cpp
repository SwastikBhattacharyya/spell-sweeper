...
size_t bloom_filter::get_size(uint32_t items_count, double fp_prob) {
...
uint32_t bloom_filter::get_hash_count(uint32_t items_count, double fp_prob) {
...
void bloom_filter::insert(std::string_view target) {
...
bool bloom_filter::lookup(std::string_view target) {
    for (uint32_t i = 0; i < this->hash_count; i++) {
        uint32_t digest;
...
bloom_filter::bloom_filter(uint32_t items_count, double fp_prob)
